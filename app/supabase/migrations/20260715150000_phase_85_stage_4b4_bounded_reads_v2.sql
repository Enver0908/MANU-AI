-- Phase 85 Stage 4B-4 remediation R7: bounded voice reads v2 and media stream metadata.

create or replace function p85_stage_4b4_load_bounded_voice_v2(
  p_tenant_id uuid,
  p_user_id uuid,
  p_dietitian_id uuid,
  p_role text,
  ...
  select jsonb_build_object(
    ...
    'audio_transcription_records', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'status', atr.status,
        'origin', atr.origin,
        'transcript_text', left(coalesce(atr.transcript_text, ''), 4096),
        'supersedes_transcription_id', atr.supersedes_transcription_id,
        'transcript_status', case
          when atr.origin = 'dietitian_correction' and atr.status = 'accepted' then 'corrected'
          when exists (
            select 1
            from audio_transcript_corrections atc
            where atc.tenant_id = atr.tenant_id
              and atc.corrected_transcription_id = atr.id
              and atc.status in ('applied_to_pending', 'applied_to_sent', 'manual_follow_up')
          ) then 'corrected'
          when atr.status = 'accepted'
            and coalesce((atr.quality_decision->>'accepted')::boolean, false) = true then
            case
              when coalesce(atr.transcript_text, '') <> '' then 'accepted'
              else 'unavailable'
            end
          when atr.status in ('review_required', 'rejected') then 'review_required'
          when atr.status = 'failed' then 'failed'
          when atr.status = 'superseded' then 'expired'
          when atr.status in ('pending', 'processing') then 'pending'
          else 'pending'
        end,
        'latest_correction_id', (
          ...
          where atc.tenant_id = atr.tenant_id
            and (
              atc.corrected_transcription_id = atr.id
              or atc.transcription_id = atr.id
              or atc.source_transcription_id = atr.id
            )
    ...
    'audio_transcript_corrections', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'source_transcription_id', atc.source_transcription_id,
        'corrected_transcription_id', atc.corrected_transcription_id,
        'target_message_id', atc.target_message_id,
        'status', atc.status,
        'corrected_transcript', left(atc.corrected_transcript, 4096),
...
begin
  ...
  return jsonb_build_object(
    ...
    'byte_size', coalesce(v_asset.byte_size, 0),
    'etag', coalesce(nullif(v_asset.content_sha256, ''), v_asset.id::text)
  );
end;
$$;

revoke all on function p85_stage_4b4_load_bounded_voice_v2(uuid, uuid, uuid, text, uuid, uuid[]) from public, anon, authenticated;
grant execute on function p85_stage_4b4_load_bounded_voice_v2(uuid, uuid, uuid, text, uuid, uuid[]) to service_role;
