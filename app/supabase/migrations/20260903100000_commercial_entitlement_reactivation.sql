-- Public surface Phase 4: append-only reactivation for revoked manual entitlements.

alter table manual_entitlement_operations
  drop constraint if exists manual_entitlement_action_check;

alter table manual_entitlement_operations
  add constraint manual_entitlement_action_check
  check (action in ('activate', 'renew', 'reactivate'));

alter table commercial_admin_audit_events
  drop constraint if exists commercial_admin_audit_event_type_check;
