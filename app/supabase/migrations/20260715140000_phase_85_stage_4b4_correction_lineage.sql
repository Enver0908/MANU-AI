-- Phase 85 Stage 4B-4 post-closure remediation R6:
-- correction lineage hardening, decision supersession, and dietitian scope validation.
...
alter table audio_transcription_records
  add constraint audio_transcription_records_dietitian_correction_confidence_check check (
    origin is distinct from 'dietitian_correction'
    or (overall_confidence is null and minimum_segment_confidence is null)
  );

create or replace function p85_stage_4b4_assert_transcript_correction_scope_v1(
  p_tenant_id uuid,
  p_dietitian_id uuid,
  p_client_id uuid,
  p_target_message_id uuid,
  p_transcription_message_id uuid
)
returns void
language plpgsql
security definer
...
begin
  ...
  if p_target_message_id is null or p_target_message_id <> p_transcription_message_id then
    raise exception 'transcript_correction_target_message_mismatch';
  end if;

  if p_dietitian_id is null then
    raise exception 'transcript_correction_dietitian_required';
  end if;
  ...
  if not exists (
    ...
    raise exception 'transcript_correction_dietitian_not_assigned';
  end if;
end;
$$;

create or replace function p85_stage_4b4_supersede_transcript_correction_decision_v1(
  p_tenant_id uuid,
  p_decision_id uuid
)
returns void
...
begin
  ...
  set send_status = 'draft_invalidated',
      blocked_reason = 'transcript_correction_superseded',
      updated_at = now()
  where tenant_id = p_tenant_id
    and id = p_decision_id
    and send_status in ('draft', 'pending_send', 'queued');
end;
$$;
...
declare
  ...
  v_outcome_version text := coalesce(p_outcome->>'version', '');
begin
  if auth.role() <> 'service_role' then
    raise exception 'service_role_required';
  end if;
  ...
  if v_outcome_version not in (
    'p85-stage-4b4-transcript-correction-outcome-v1',
    'p85-stage-4b4-transcript-correction-outcome-v2'
  ) then
  ...
  v_target_message_id := coalesce(
    nullif(p_outcome #>> '{correction,targetMessageId}', '')::uuid,
    v_transcription.message_id
  );

  perform p85_stage_4b4_assert_transcript_correction_scope_v1(
    p_tenant_id,
    nullif(p_outcome #>> '{correction,dietitianId}', '')::uuid,
    v_transcription.client_id,
    v_target_message_id,
    v_transcription.message_id
  );
  ...
  if p_outcome ? 'correctedTranscription' then
    ...
    select
      ...
      case
        when coalesce(p_outcome #>> '{correctedTranscription,origin}', 'dietitian_correction') = 'dietitian_correction'
          then null
        else p_outcome #> '{correctedTranscription,observation}'
      end,
  ...
  perform p85_stage_4b4_supersede_transcript_correction_decision_v1(
    p_tenant_id,
...
revoke all on function p85_stage_4b4_assert_transcript_correction_scope_v1(uuid, uuid, uuid, uuid, uuid) from public, anon, authenticated;
revoke all on function p85_stage_4b4_supersede_transcript_correction_decision_v1(uuid, uuid) from public, anon, authenticated;

grant execute on function p85_stage_4b4_assert_transcript_correction_scope_v1(uuid, uuid, uuid, uuid, uuid) to service_role;
grant execute on function p85_stage_4b4_supersede_transcript_correction_decision_v1(uuid, uuid) to service_role;
