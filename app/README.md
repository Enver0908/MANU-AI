...
**Current domain cutover authority (2026-09-01):** The hosted commercial entry surfaces are prepared to move from `siriusai.store` to `aiyaworkspace.com`. Public/customer canonical URL is `https://aiyaworkspace.com`, admin host is `https://admin.aiyaworkspace.com`, and public purchase/contact mail is `contact@aiyaworkspace.com`. Legacy `siriusai.store` is not retained as a long-term redirect domain because there is no real customer traffic. PWA installs must be recreated from the new origin. Runbook: `../docs/AIYAWORKSPACE_DOMAIN_CUTOVER_RUNBOOK.md`.
