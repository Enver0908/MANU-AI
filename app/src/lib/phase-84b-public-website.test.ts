...
describe("phase 84b public website", () => {
  it("uses the public business contact email", () => {
    expect(AIYA_MARKETING_CONTACT_EMAIL).toBe("contact@aiyaworkspace.com");
  });

  it("builds a mailto url with encoded subject", () => {
    expect(buildContactMailtoUrl("AIya erişim talebi")).toBe(
      "mailto:contact@aiyaworkspace.com?subject=AIya+eri%C5%9Fim+talebi",
    );
  });
