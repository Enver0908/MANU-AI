import {
  ...
  parseAiChatRegenerateMessageBody,
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const input = parseAiChatRegenerateMessageBody(body);
    requestId = input.requestId;
    return await withAiChatRoute(
      "mutation",
      async (tenantContext, store) => {
        const result = await store.regenerateMessage(tenantContext, messageId, input);
