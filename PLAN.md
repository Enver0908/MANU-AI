...
The canonical planning baseline is `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_PLAN.md`. P85-IF-A added `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_SPEC.md` as the implementation contract and threat model for verified WhatsApp business-human actor provenance, complete transcript persistence, tenant/client/counterparty routing, historical retrieval beyond the last-eight-message window, human/AI coordination, authenticated yellow/red resolution through direct AI reactivation, and controlled off-channel AI Chat context intake. P85-IF-B added the trust-root/provenance data model foundation: nullable message provenance fields, canonical account/actor/event/revision/session/risk/context-intake records, Supabase mappers, append-only migration, and focused contract tests. Runtime provider/channel behavior remains closed. The next operator action is P85-IF-C. P85-IF provides stable alert/notification semantics and minimal verification UI only; the full alert/notification product experience remains Phase 85 Stage 4B scope.
...
P85-IF-C added the secure ingress, ledger, routing, and quarantine engine: `phase-85-if-c-channel-event-normalizer.ts`, `phase-85-if-c-channel-event-routing.ts`, and `phase-85-if-c-channel-event-ledger.ts`. Normalization decomposes mock WhatsApp Cloud-style webhook payloads into independent candidates per entry/change/message/status/echo/history item and derives the exact event kind from provider fields only. Routing implements the mandatory fail-closed account-binding -> tenant -> counterparty/client -> lifecycle/channel -> actor -> conversation order. The ledger adds a secure mock-webhook gate (`MANU_ALLOW_MOCK_WHATSAPP_WEBHOOK` plus `MANU_MOCK_WHATSAPP_WEBHOOK_SECRET`, refusing production and `MANU_HOSTED_SANDBOX_ACTIVE`), duplicate event/message idempotency, quarantine plus seven-day mock replay/expiry, and audit trail. Only fully-resolved `client_message_text` events delegate to the existing, unmodified orchestrator path; all other event kinds are ledger-recorded only. This engine is additive and not yet wired into the live `/api/whatsapp/webhook` route because no account binding is seeded by default in the fallback/demo state; storing business-human echoes, auto-pausing AI, and human-control session integration remain P85-IF-D scope. Verification on 2026-07-10: targeted P85-IF-C Vitest 26/26, adjacent regression spot-checks green, lint 0 errors (3 pre-existing warnings, unchanged), `git diff --check` clean, secret/token scan clean, Phase 86 naming scan clean. A full `npm test` and `npm run build` re-run is still owed: the verification sandbox could not complete either (suite duration and an outbound Google Fonts network restriction unrelated to the code). `npm run test:rls` was not re-run. Production pilot remains `NO-GO`; R-405 remains open; R-406 current local Supabase/RLS re-run remains pending. P85-IF-D is next.
