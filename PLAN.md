...
**Current authority (2026-08-24):** Stage 6 is locally `STAGE_6_CLOSED`. Android Chrome/PWA physical evidence and final `release:verify` passed. Physical iPhone Safari/PWA was explicitly waived as `WAIVED_NOT_EXECUTED`, not represented as PASS, and remains required before any iOS production pilot/readiness claim. Stage 7.1 through Stage 7.4 are superseded for Stage 7 closure. Stage 7R.0 and Stage 7R.1 are complete locally; Stage 7R.2 trusted baseline is recorded and awaits user visual approval. Stage 7 is not closed, Stage 7.5 is not started, and Stage 7R.3 must not start before visual approval or a user-directed adjustment. Stage 5 remains closed; production remains `NO-GO`. Stage 7R authority: `docs/PHASE_85_STAGE_7R_SUPERSESSION_DECISION.json`; finding lock: `docs/PHASE_85_STAGE_7R_FINDING_LOCK.json`.
