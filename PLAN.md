...
4. Keep Phase 23-41 prompt-context, voice-profile, dynamic-form, internal-copilot, dietitian context update, provider-boundary, channel-policy, incident/DSAR, backup/restore, secret-rotation, dependency-audit, send-revalidation, evidence, external-approval, legal/privacy, clinical-taxonomy, provider/vendor, and RLS controls covered by tests before any real provider or channel integration.
5. Use `docs/PRODUCTION_PILOT_FINAL_READINESS_CLOSURE_SUMMARY.md` as the current go/no-go summary until external approvals, R-405, and R-406 change.
