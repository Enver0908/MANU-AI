...
Phase 85 Interstage Foundation P85-IF-A/P85-IF-B on 2026-07-10 adds `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_SPEC.md` as the canonical implementation contract and threat model after `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_PLAN.md` inserted P85-IF between completed Stage 4A and Stage 4B, then implements the trust-root/provenance data model foundation with nullable message provenance, canonical account/actor/event/revision/session/risk/context-intake records, Supabase mappers, append-only migration, and focused contract tests. P85-IF-C is next, and Stage 4B resumes after P85-IF-I. This foundation checkpoint does not approve production pilot, close launch gates, resolve R-405/R-406, or connect real provider/channel/monitoring/backup/secret-manager/real health-data paths.

Phase 85 Interstage Foundation P85-IF-C on 2026-07-10 implements the secure ingress, ledger, routing, and quarantine engine (`phase-85-if-c-channel-event-normalizer.ts`, `phase-85-if-c-channel-event-routing.ts`, `phase-85-if-c-channel-event-ledger.ts`): batch-safe normalization deriving the exact event kind from provider fields only, mandatory fail-closed account-binding/tenant/client/actor/conversation routing with exclusive/shared business-actor resolution, a secure mock-webhook gate requiring a feature flag and secret with production/hosted-sandbox refusal, duplicate event/message idempotency, and quarantine plus seven-day mock replay/expiry. Only fully-resolved `client_message_text` events delegate to the existing, unmodified orchestrator path; the engine is additive and intentionally not wired into the live `/api/whatsapp/webhook` route in this track. Storing business-human echoes as verified `dietitian_manual`, auto-pausing AI, and human-control session integration remain P85-IF-D scope. Verification: targeted tests 26/26, adjacent regression spot-checks green, lint 0 errors (3 pre-existing warnings, unchanged), `git diff --check`/secret scan/Phase 86 naming scan clean; a full `npm test` and `npm run build` re-run remains owed because the verification sandbox could not complete either (suite duration and an outbound Google Fonts network restriction unrelated to the code). This foundation checkpoint does not approve production pilot, close launch gates, resolve R-405/R-406, or connect real provider/channel/monitoring/backup/secret-manager/real health-data paths.
