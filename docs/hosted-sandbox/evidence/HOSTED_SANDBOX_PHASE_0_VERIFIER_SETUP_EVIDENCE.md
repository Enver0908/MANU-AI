...
Faz 0 installed Hosted Sandbox verifier scripts and captured a baseline identity/migration fingerprint. It did not change product runtime behavior, migrations, package manifests, lockfiles, production gates, or deployment behavior.

## Planning Status

This evidence is preserved as historical Hosted Sandbox setup evidence. The later machine-lock governance package system was removed on 2026-08-26; future planning returns to the Phase 85 style of Markdown plans, evidence files, targeted checks, explicit user approval, and normal Git review.
...
Runtime artifacts under `.manu-runtime/` must not be committed.
