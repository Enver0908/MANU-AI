...
- Current `next@latest` is `16.2.6` and still depends on `postcss@8.4.31`.
- Current `next@canary` is `16.3.0-canary.32` and depends on `postcss@8.5.10`, but canary is not accepted as a pilot baseline.
