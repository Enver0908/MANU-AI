...
Phase 77J added ephemeral in-memory DOCX/PDF menu export binaries generated from active export-visible menu plans via `/api/clients/[id]/menu-plans/export`; exports exclude internal plan metadata (`dietitianNotes`, catalog checksums, revision fields). Phase 74 full client export bumped to `phase74-export-v1.2` with `personal_form_v2.json`, `catalog_version_refs.json`, and deprecated proposal export sections.
