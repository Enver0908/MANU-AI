...
- Latest local release verification: `npm run release:verify` passed on 2026-06-03 after Phase 60 audit remediation.
- Verification result: core tests 104/104, app tests 138/138, lint passed, production build passed.
...
| Qualified dietitian clinical taxonomy approval | Qualified dietitian sign-off, current clinical golden test report, taxonomy change log, and production clinical safety second-layer or equivalent fail-closed evaluation evidence | `PRODUCTION_PILOT_CLINICAL_TAXONOMY_REVIEW_PACKET.md`, `CLINICAL_TAXONOMY_REVIEW_WORKFLOW.md`, clinical JSONL golden cases (30 cases, `dietetic-risk-v0.3.1`), Phase 44 red-risk reactivation lock, Phase 55–56 safety evidence, Phase 59–60 glucose/symptom hardening, 104 core tests, persona-invariant tests | Qualified dietitian must approve current taxonomy version, red/yellow/green routing, multilingual routing, prompt-injection escalation behavior, red-risk reactivation policy, and the production safety evaluation approach. The deterministic/regex classifier is a local first barrier and is not sufficient as the sole production clinical safety layer. | Signed taxonomy review record naming taxonomy version, test result, and approved second-layer or equivalent fail-closed safety evaluation approach | Open |
