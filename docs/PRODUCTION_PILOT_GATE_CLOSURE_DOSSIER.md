...
| Qualified dietitian clinical taxonomy approval | Qualified dietitian sign-off, current clinical golden test report, taxonomy change log, approved regulation corpus for scope guard, and production clinical safety second-layer or equivalent fail-closed evaluation evidence | `PRODUCTION_PILOT_CLINICAL_TAXONOMY_REVIEW_PACKET.md`, `CLINICAL_TAXONOMY_REVIEW_WORKFLOW.md`, clinical JSONL golden cases (30 cases, `dietetic-risk-v0.3.1`), Phase 44 red-risk reactivation lock, Phase 55–56 safety evidence, Phase 59–60 glucose/symptom hardening, Phase 61 mock-first scope guard (`PHASE_61_SCOPE_GUARD_RAG_SECOND_LAYER_SPEC.md`), 112 core tests, persona-invariant tests | Qualified dietitian must approve current taxonomy version, red/yellow/green routing, multilingual routing, prompt-injection escalation behavior, red-risk reactivation policy, approved scope regulation corpus chunks, and the production safety evaluation approach. The deterministic/regex classifier is a local first barrier and is not sufficient as the sole production clinical safety layer. | Signed taxonomy review record naming taxonomy version, test result, approved scope corpus version, and approved second-layer or equivalent fail-closed safety evaluation approach | Open |
