...
- Git baseline: local working tree contains Phase 21-26 changes awaiting the user's checkpoint/commit decision.
- Latest local verification: `npm run release:verify` passed on 2026-05-30 after Phase 26.
- Verification result: core tests 39/39, app tests 96/96, lint passed, production build passed.
- Dependency audit result: only known R-405 findings, `next:postcss` and `postcss:GHSA-qx2v-qp2m-jg93`.
- R-405 status: open production launch blocker; no safe stable Next.js/PostCSS patch path applied.
...
| --- | --- | --- | --- | --- | --- |
| Legal and privacy review | Legal basis matrix, privacy notice and client permission documents, medical-device or clinical-decision-support classification memo | `DATA_INVENTORY.md`, `PHASE_5_DATA_GOVERNANCE_SPEC.md`, `PHASE_14_DSAR_RETENTION_LEGAL_OPS_SPEC.md`, `PHASE_26_INTERNAL_COPILOT_SPEC.md`, tenant/client-scoped export and anonymization tests | Legal/privacy counsel must approve lawful basis, privacy notice, permission flow, data retention, internal copilot records, and classification | Signed legal/privacy memo or dated counsel approval record | Open |
| Qualified dietitian clinical taxonomy approval | Qualified dietitian sign-off, current clinical golden test report, taxonomy change log | `CLINICAL_TAXONOMY_REVIEW_WORKFLOW.md`, clinical JSONL golden cases, 39 core tests, persona-invariant tests | Qualified dietitian must approve `dietetic-risk-v0.2.0`, red/yellow/green routing, and escalation behavior | Signed taxonomy review record naming taxonomy version and test result | Open |
| Provider vendor and retention review | Provider terms review, health-data retention configuration, prompt and completion logging decision | `AI_PROVIDER_REQUIREMENTS.md`, local mock provider, provider input allowlist guard, provider failure no-send behavior, Phase 26 local/mock-only copilot boundary | Vendor/legal review must approve any Gemini or external LLM use with health data, including any future internal copilot provider egress | Vendor-risk approval record covering retention, training use, logging, region, and access controls | Open |
...
| Production dependency audit clearance | Production dependency audit report, R-405 resolution or formal acceptance | `PHASE_19_RELEASE_VERIFICATION_DEPENDENCY_GATE_SPEC.md`, `PHASE_22_R405_DEPENDENCY_REMEDIATION_SPEC.md`, `npm run release:verify`, `RISK_REGISTER.md` | R-405 must be resolved through a safe stable patch path or formally accepted before pilot | Clean production audit report, safe stable upgrade evidence, or formal risk acceptance | Open |

## Review Packet Checklist

- Legal/privacy packet: data inventory, permission integration points, DSAR/export/anonymization evidence, retention placeholder list.
...
- Provider packet: provider requirements, no-storage/no-training requirements, prompt/completion logging decision checklist, provider input allowlist.
