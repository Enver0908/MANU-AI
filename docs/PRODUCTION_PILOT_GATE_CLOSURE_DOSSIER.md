...
- Latest local release verification: `npm run release:verify` passed on 2026-05-31 after Phase 43 multilingual language support.
- Verification result: core tests 52/52, app tests 107/107, lint passed, production build passed.
...
| Qualified dietitian clinical taxonomy approval | Qualified dietitian sign-off, current clinical golden test report, taxonomy change log | `PRODUCTION_PILOT_CLINICAL_TAXONOMY_REVIEW_PACKET.md`, `CLINICAL_TAXONOMY_REVIEW_WORKFLOW.md`, clinical JSONL golden cases including Phase 43 multilingual cases, 52 core tests, persona-invariant tests | Qualified dietitian must approve current taxonomy version, red/yellow/green routing, multilingual routing, and escalation behavior | Signed taxonomy review record naming taxonomy version and test result | Open |
