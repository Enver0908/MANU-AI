...
Phase 85 roadmap and interstage update (2026-07-10): completed Stage 4A is followed by the mandatory Phase 85 Interstage Foundation (`P85-IF`) before Stage 4B. `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_PLAN.md` is the planning baseline, and P85-IF-A added `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_SPEC.md` as the canonical contract/threat-model evidence. P85-IF-B added the trust-root/provenance data model foundation with nullable message provenance, canonical channel/account/event/revision/session/risk/context-intake records, Supabase mappers, append-only migration, and focused contract tests. P85-IF-C is next, and Stage 4B resumes after P85-IF-I. This does not approve production pilot, close launch gates, resolve R-405/R-406, or connect real provider/channel/monitoring/backup/secret-manager/real health-data paths.
...
- P85-IF-C on 2026-07-10: secure ingress, ledger, routing, and quarantine engine implemented (batch-safe normalization, fail-closed account/client/actor/conversation routing, secure mock-webhook secret gate, duplicate idempotency, quarantine plus seven-day mock replay/expiry); only fully-resolved `client_message_text` events delegate to the existing orchestrator path and the engine is not yet wired into the live webhook route; targeted tests 26/26, lint 0 errors (3 pre-existing warnings); full `npm test`/`npm run build` re-run still owed (sandbox suite-duration and network limits, not code defects); production pilot remains `NO-GO`.
