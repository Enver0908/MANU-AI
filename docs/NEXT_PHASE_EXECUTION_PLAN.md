...
**Current authority (2026-08-21):** Stage 6 is locally `STAGE_6_CLOSED`. Android Chrome/PWA physical evidence and final `release:verify` passed. New physical iPhone Safari/PWA evidence was explicitly waived as `WAIVED_NOT_EXECUTED`; it is not a PASS and must be completed before any iOS production pilot/readiness claim. No Stage 7 implementation is authorized by this closure. Stage 5 remains closed and production remains `NO-GO`. Authority: `docs/PHASE_85_STAGE_6_CLOSURE_DECISION.json`; evidence: `docs/PHASE_85_STAGE_6_FINAL_CLOSURE_EVIDENCE.md`.
