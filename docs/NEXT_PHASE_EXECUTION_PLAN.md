...
Current Phase 85 staging update (2026-07-10): Stages 1-3 and Stage 4A are complete. The mandatory Phase 85 Interstage Foundation (`P85-IF`) is inserted between Stage 4A and Stage 4B and is canonically planned in `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_PLAN.md`. P85-IF-A added `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_SPEC.md` as the canonical provider contract, threat model, state-machine, prompt-authority, and Stage 4B boundary implementation contract. P85-IF-B added the trust-root/provenance data model foundation with nullable message provenance, canonical account/actor/event/revision/session/risk/context-intake records, Supabase mappers, append-only migration, and focused contract tests. Runtime provider/channel behavior remains closed, and P85-IF-C is the next operator action. After P85-IF-I closure, execution returns to Stage 4B Uyari ve Bildirimler, followed by Stage 4C, Stage 4D, Stage 5, Stage 6, and Stage 7.

Current P85-IF-C update (2026-07-10): the secure ingress, ledger, routing, and quarantine engine is implemented in `app/src/lib/phase-85-if-c-channel-event-normalizer.ts`, `phase-85-if-c-channel-event-routing.ts`, and `phase-85-if-c-channel-event-ledger.ts`. Normalization decomposes mock WhatsApp Cloud-style webhook payloads into independent candidates per entry/change/message/status/echo/history item and derives the exact event kind from provider fields only. Routing implements the mandatory fail-closed account-binding -> tenant -> counterparty/client -> lifecycle/channel -> actor -> conversation order. The ledger adds a secure mock-webhook gate (feature flag plus `MANU_MOCK_WHATSAPP_WEBHOOK_SECRET`, refusing production and hosted-sandbox execution), duplicate event/message idempotency, quarantine plus seven-day mock replay/expiry, and audit trail. Only fully-resolved `client_message_text` events delegate to the existing, unmodified orchestrator path; the engine is additive and not yet wired into the live `/api/whatsapp/webhook` route because no account binding is seeded by default in the fallback/demo state. Storing business-human echoes, auto-pausing AI, and human-control session integration remain P85-IF-D scope. Verification: targeted P85-IF-C tests 26/26, adjacent regression spot-checks green, lint 0 errors (3 pre-existing warnings, unchanged), `git diff --check` clean, secret/token scan clean, Phase 86 naming scan clean. A full `npm test`/`npm run build` re-run is still owed: the verification sandbox could not complete either (suite duration and an outbound Google Fonts network restriction unrelated to the code). `npm run test:rls` was not re-run. Production pilot remains `NO-GO`; R-405 remains open; R-406 current local Supabase/RLS re-run remains pending. P85-IF-D is next.
...
Current Phase 85 Stage 4A update (2026-07-08): Stage 4A.1 through Stage 4A.4 are implemented; Stage 4A Danisan Kontrol Paneli is complete. Verification for Stage 4A.4: lint 0 errors (3 pre-existing warnings), helper tests 4/4, full app suite 734 passed / 4 skipped, build passed, Playwright visual 36/36. P85-IF planning later superseded Stage 4B as the immediate next action; P85-IF-A and P85-IF-B are now complete and P85-IF-C is next, with Stage 4B resuming after P85-IF closure. Production pilot remains `NO-GO`; R-405 and current RLS re-run remain open.
