...
**Current authority (2026-08-24):** Stage 6 is locally `STAGE_6_CLOSED`. Android Chrome/PWA physical evidence and final `release:verify` passed. New physical iPhone Safari/PWA evidence was explicitly waived as `WAIVED_NOT_EXECUTED`; it is not a PASS and must be completed before any iOS production pilot/readiness claim. Stage 7.1 through Stage 7.4 are superseded for Stage 7 closure. Stage 7R.0 and Stage 7R.1 are complete locally; Stage 7R.2 recorded the trusted deterministic baseline and now waits for explicit user visual approval of `docs/PHASE_85_STAGE_7R_VISUAL_APPROVAL_MANIFEST.json`. Stage 7 is not closed, Stage 7.5 is not started, and Stage 7R.3 must not start until the visual set is approved or adjusted. Stage 5 remains closed and production remains `NO-GO`. Authority: `docs/PHASE_85_STAGE_7R_SUPERSESSION_DECISION.json`; finding lock: `docs/PHASE_85_STAGE_7R_FINDING_LOCK.json`.
