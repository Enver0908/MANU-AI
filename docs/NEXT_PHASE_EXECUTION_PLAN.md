...
**Current public surface/auth/onboarding/PWA execution lock (2026-09-04):** Canonical plan remains `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_ACTION_PLAN.md`. Phase 5 public site, CTA, brand, and metadata polish is locally closed in `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_PHASE_5_PUBLIC_SITE_CTA_BRAND_METADATA_EVIDENCE.md`. Public CTAs are “Bize ulaşın” and “Giriş yap”; `/purchase` remains a direct-URL remnant; customer/admin canonical origins are separated; visible tenant fallback is `AIya Workspace`. The local tenant-fallback SQL file is not a remote apply. The next eligible unit is Phase 6 (PWA, app-install, responsive, and accessibility) only after explicit user approval. Production remains `NO-GO`; no deploy, remote migration, provider/channel egress, live billing, external system edit, or production gate change is authorized.
