...
Current Phase 85 Stage 4A planning update (2026-07-08): `docs/PHASE_85_STAGE_4A_CLIENT_CONTROL_PANEL_ARCHITECTURE_PLAN.md` is created after code-level review of client form, active nutrition rules, menu/export, and AI assistant control contracts. The next dashboard work is now organized around the dietitian's per-client service surface: DCP-1 full client form editor, DCP-2 active nutrition plan/catalog rules, DCP-3 menu workflow/export, and DCP-4 AI assistant control. This is planning/spec only; no runtime behavior changed. Next operator action is to request user approval before implementing DCP-1. Production pilot remains `NO-GO`; R-405 and current RLS re-run remain open.
