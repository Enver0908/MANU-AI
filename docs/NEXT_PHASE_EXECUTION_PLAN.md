...
**Current public surface/auth/onboarding/PWA execution lock (2026-09-04):** Canonical plan remains `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_ACTION_PLAN.md`. Phase 6 PWA, app-install, responsive, and accessibility work is locally closed in `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_PHASE_6_PWA_INSTALL_RESPONSIVE_A11Y_EVIDENCE.md`. AIya PWA stays network-only and offline privacy-locked; `/app-install` remains the only install route. Local Android Chrome and installed-PWA checks PASS on Playwright Chromium Pixel 5; TalkBack and iPhone remain `WAIVED_NOT_EXECUTED`. The next eligible unit is Phase 7 (frontend remnant cleanup and unified local closure) only after explicit user approval. Production remains `NO-GO`; no deploy, remote migration, provider/channel egress, live billing, external system edit, or production gate change is authorized.
