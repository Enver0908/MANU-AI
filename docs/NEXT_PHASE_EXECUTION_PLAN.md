...
**Current public surface/auth/onboarding/PWA execution lock (2026-09-03):** Canonical plan remains `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_ACTION_PLAN.md`. Phase 2 two-hour secure session continuity is locally closed in `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_PHASE_2_TWO_HOUR_SESSION_EVIDENCE.md`. Authenticated web/PWA idle timeout is two hours and server-authoritative; the local migration is not a production apply. The next eligible unit is Phase 3 (password-first login and invite onboarding) only after explicit user approval. Production remains `NO-GO`; no deploy, remote migration, provider/channel egress, live billing, external system edit, or production gate change is authorized.
