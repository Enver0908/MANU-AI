...
**Current AIya hosted runtime status (2026-09-01):** The hosted VPS now serves commit `82ee3725076566304e9e0308632b2efe9d3b1deb` as release `hs-82ee37250765-2c32cf194421`. Live AIya smoke passed for primary public/admin routes, PWA manifest, `/app-install`, unauthenticated app-state `401` fail-closed responses, and legacy-domain `410`. This does not approve the direct 100-dietitian pilot, close production launch gates, connect providers/channels, enable live billing, roll out production schema, or process real health data. Next technical step is hosted deploy repeatability: remote deploy helper plus Linux-portable release artifacts. Evidence: `docs/AIYA_HOSTED_DEPLOY_LIVE_SMOKE_EVIDENCE.md`.
