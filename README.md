...
**2026-07-12 Stage 4B-2 Phase 1:** Domain, bounded DTO, actor/assignment authorization projection, safe previews, sequence unread calculation, deterministic cursors, and fallback-compatible conversation list/detail projections are implemented. Dedicated Phase 1 tests passed 8/8; the combined Stage 4B regression set passed 61 with 1 skipped; core passed 234/234; build passed; lint has 0 errors and 3 pre-existing warnings. RLS remains environment-skipped because Docker/Supabase is unavailable and is not counted as pass. Evidence: `docs/PHASE_85_STAGE_4B_2_PHASE_1_DOMAIN_DTO_AUTHORIZATION_EVIDENCE.md`. Receipt persistence/RLS is next; routes, UI, mutations, Stage 4C, production pilot, R-405, and real provider/channel/health-data paths remain closed.
