...
**Hosted Sandbox technical-debt update (2026-08-27):** The old Hosted Sandbox Remediation v1.1 governance plan is superseded. Local implementation is closed for release health, artifact hash verification, exact deploy smoke, rollback, Nginx maintenance flag, backup manifest freshness/hash gates, cleanup apply safeguards, migration workflow apply preparation, and SSH-pinned local deploy operator preparation. `npm run release:verify` passed. Remote execution is not claimed: Docker, Supabase CLI, `pg_dump`, `age`, hosted secrets, SSH known_hosts pin, and explicit remote approvals were unavailable in this session. Production remains `NO-GO`; physical iPhone Safari/PWA remains `WAIVED_NOT_EXECUTED`, not PASS. Evidence: `docs/hosted-sandbox/evidence/HOSTED_SANDBOX_TECHNICAL_DEBT_CLOSURE_EVIDENCE.md`.
