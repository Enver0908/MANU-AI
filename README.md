...
**2026-07-10 P85-IF-C secure ingress, ledger, routing, and quarantine:** Added the Phase 85 Interstage Foundation ingress engine: `phase-85-if-c-channel-event-normalizer.ts` decomposes mock WhatsApp Cloud-style webhook payloads into independent normalized channel event candidates per entry/change/message/status/echo/history item, deriving the exact event kind from provider fields only (never message text); `phase-85-if-c-channel-event-routing.ts` implements the mandatory fail-closed resolution order (account binding -> tenant -> counterparty/client -> lifecycle/channel -> actor -> conversation), covering unknown-account, unknown-client, ambiguous-client, cross-tenant-collision, and revision-unknown-target quarantine plus exclusive/shared business-actor resolution; `phase-85-if-c-channel-event-ledger.ts` adds the secure mock-webhook gate (`MANU_ALLOW_MOCK_WHATSAPP_WEBHOOK` plus `MANU_MOCK_WHATSAPP_WEBHOOK_SECRET`, with production and `MANU_HOSTED_SANDBOX_ACTIVE` refusal), duplicate event/message idempotency, quarantine plus seven-day mock replay/expiry, and audit trail. Only fully-resolved `client_message_text` events delegate to the existing, unmodified `processMockChannelInbound` orchestrator path, so current client-facing AI/risk behavior is unchanged; all other event kinds are ledger-recorded only. This ingress engine is additive and is not yet wired into the live `/api/whatsapp/webhook` route; storing business-human echoes as verified `dietitian_manual`, auto-pausing AI, and human-control session integration remain P85-IF-D scope. Verification: targeted P85-IF-C tests 26/26, adjacent channel/copilot/context/auth/governance regression spot-checks green, lint 0 errors (3 pre-existing warnings, unchanged), `git diff --check` clean, secret/token scan clean, Phase 86 naming scan clean; production build was blocked in the verification sandbox by an outbound network restriction to Google Fonts unrelated to this change, and repo-wide `npm test` exceeded the local sandbox timeout (a pre-existing documented condition), so a full green `npm test`/build re-run is still owed before P85-IF closure. Production pilot remains `NO-GO`; R-405 remains open; R-406 current local Supabase/RLS re-run remains pending. P85-IF-D is next.

**2026-07-10 P85-IF-B trust-root and provenance data model:** Added the Phase 85 Interstage Foundation trust-root/provenance model: TypeScript records for channel account bindings, actor bindings, channel events, message revisions, human-control sessions, risk activity events, and context-intake proposals; nullable `MessageRecord` provenance extensions; fallback/Supabase mapper support; append-only migration `20260710120000_phase_85_if_b_trust_root_provenance.sql`; and a focused provenance model test. This is schema/model foundation only: real provider/channel/health-data paths remain disconnected, existing message behavior is unchanged, and P85-IF-C is next. Production pilot remains `NO-GO`; R-405 remains open; R-406 current local Supabase/RLS re-run remains pending.
...
**Latest implementation phase:** Phase 85 Stage 4A Danisan Kontrol Paneli is complete (Stage 4A.1-4A.4), P85-IF-A and P85-IF-B are complete, and P85-IF-C has added the secure ingress, ledger, routing, and quarantine engine (not yet wired into the live webhook route). **Next work:** P85-IF-D complete transcript and human control, followed by P85-IF-E through P85-IF-I. After P85-IF closure, return to Stage 4B Uyari ve Bildirimler, then Stage 4C, Stage 4D, Stage 5, Stage 6, and Stage 7. **Production pilot:** `NO-GO` (unchanged).
...
**Latest verification:** P85-IF-C targeted tests passed 26/26, lint had 0 errors with 3 pre-existing warnings (unchanged), `git diff --check`/secret scan/Phase 86 naming scan were clean; the verification sandbox could not complete a full `npm test`/build pass (Google Fonts network restriction and suite-duration limits unrelated to this change), so full `npm test` and `npm run build` are still owed before P85-IF closure. Prior P85-IF-B verification remains: targeted tests 6/6, app `npm test` 740 / 4 skipped, core `npm test` 225/225, `npm run test:rls` skipped 21/21 because local Supabase was unavailable. Production pilot remains `NO-GO`.
