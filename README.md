...
**Hosted Sandbox technical-debt update (2026-08-28):** The old Hosted Sandbox Remediation v1.1 governance plan is superseded. Local implementation is closed for release health, artifact hash verification, exact deploy smoke, rollback, Nginx maintenance flag, backup manifest freshness/hash gates, cleanup apply safeguards, migration workflow apply preparation, SSH-pinned local deploy operator preparation, clean local Supabase reset, and zero-skip RLS. `npm run release:verify`, `npx supabase db reset`, and `npm run test:rls` passed with RLS 56/56 and 0 skipped. Remote hosted execution is not claimed: `pg_dump`, `pg_restore`, `age`, hosted secrets, SSH known_hosts pin, and explicit remote approvals were unavailable in this session. Production remains `NO-GO`; physical iPhone Safari/PWA remains `WAIVED_NOT_EXECUTED`, not PASS. Evidence: `docs/hosted-sandbox/evidence/HOSTED_SANDBOX_TECHNICAL_DEBT_CLOSURE_EVIDENCE.md`.
