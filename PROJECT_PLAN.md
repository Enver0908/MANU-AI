...
Phase 85 Stage 4A client control panel architecture update, 2026-07-08: `docs/PHASE_85_STAGE_4A_CLIENT_CONTROL_PANEL_ARCHITECTURE_PLAN.md` is created from code review of the current client form, food-rule profile, menu plan/export, and AI activation/preflight architecture. The dashboard work is now re-scoped around dietitian service quality for each client: DCP-1 full client form editor, DCP-2 active nutrition plan/catalog rules, DCP-3 menu workflow/export, and DCP-4 AI assistant control. This is a planning/spec update only; implementation remains pending explicit user approval for each sub-phase. No runtime behavior or launch-gate state changed.
