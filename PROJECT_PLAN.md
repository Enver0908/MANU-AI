...
Phase 85 Interstage Foundation update, 2026-07-10: `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_PLAN.md` inserts P85-IF between completed Stage 4A and Stage 4B, and `docs/PHASE_85_INTERSTAGE_TRUSTED_CLINICAL_COMMUNICATION_MEMORY_SPEC.md` now provides the P85-IF implementation contract/threat model. P85-IF is a cross-cutting channel provenance, complete transcript, historical retrieval, human/AI coordination, risk-reactivation, and off-channel context-intake foundation program; it is not a new numbered phase and does not replace Phase 85. P85-IF-A is complete. P85-IF-B added the trust-root/provenance data model foundation with nullable message provenance fields, canonical account/actor/event/revision/session/risk/context-intake records, Supabase mappers, append-only migration, and focused contract tests. Runtime provider/channel behavior remains closed. After P85-IF-I closure, execution returns to Stage 4B. Production pilot remains `NO-GO`; R-405 remains open; R-406 current local Supabase/RLS re-run remains pending.

Phase 85 Interstage Foundation P85-IF-C update, 2026-07-10: added the secure ingress, ledger, routing, and quarantine engine (`app/src/lib/phase-85-if-c-channel-event-normalizer.ts`, `phase-85-if-c-channel-event-routing.ts`, `phase-85-if-c-channel-event-ledger.ts`). Batch-safe normalization derives the exact `ChannelEventKind` per entry/change/message/status/echo/history item from provider fields only, never from message text. Routing implements the mandatory fail-closed account-binding -> tenant -> counterparty/client -> lifecycle/channel -> actor -> conversation order, covering unknown-account, unknown-client, ambiguous-client, cross-tenant-collision, and revision-unknown-target quarantine plus exclusive/shared business-actor resolution. The ledger adds a secure mock-webhook gate (`MANU_ALLOW_MOCK_WHATSAPP_WEBHOOK` plus `MANU_MOCK_WHATSAPP_WEBHOOK_SECRET`, refusing production and `MANU_HOSTED_SANDBOX_ACTIVE`), duplicate event/message idempotency, quarantine plus seven-day mock replay/expiry, and audit trail. Only fully-resolved `client_message_text` events delegate to the existing, unmodified `processMockChannelInbound` orchestrator path; all other event kinds are ledger-recorded only. This engine is additive and intentionally not wired into the live `/api/whatsapp/webhook` route in this track, because no `ChannelAccountBindingRecord` is seeded in the fallback/demo state by default; storing business-human echoes as verified `dietitian_manual`, auto-pausing AI, and human-control session integration remain P85-IF-D scope. Supabase-side ledger persistence was deferred, matching the P85-IF-C acceptance criteria (no Supabase requirement listed, unlike P85-IF-D/E). Verification: targeted P85-IF-C Vitest 26/26, adjacent channel/copilot/context/auth/governance regression spot-checks green, lint 0 errors (3 pre-existing warnings, unchanged), `git diff --check` clean, secret/token scan clean, Phase 86 naming scan clean. A full `npm test` and `npm run build` re-run is still owed before P85-IF-C is considered fully closed: the verification sandbox could not complete either (repo-wide suite duration exceeds the local sandbox timeout, a pre-existing documented condition, and `npm run build` was blocked by an outbound network restriction to Google Fonts unrelated to the code change). `npm run test:rls` was not re-run. Production pilot remains `NO-GO`; R-405 remains open; R-406 current local Supabase/RLS re-run remains pending. P85-IF-D is next.
