...
**Current AIya hosted runtime status (2026-09-01):** The hosted VPS now serves commit `82ee3725076566304e9e0308632b2efe9d3b1deb` as release `hs-82ee37250765-2c32cf194421`. Live AIya smoke passed for primary public/admin routes, PWA manifest, `/app-install`, unauthenticated app-state `401` fail-closed responses, and legacy-domain `410`. This does not change production `NO-GO`; next technical work is repeatable hosted deploy helper installation plus Linux-portable release artifacts. Evidence: `docs/AIYA_HOSTED_DEPLOY_LIVE_SMOKE_EVIDENCE.md`.
...
**Current authority (2026-08-24, iOS waiver updated 2026-08-28):** Stage 6 Dashboard Core Workflows is locally `STAGE_6_CLOSED`. Android Chrome/PWA physical evidence passed and final `release:verify` passed. Physical iPhone Safari/PWA was not executed and is explicitly recorded as `WAIVED_NOT_EXECUTED`; this is an owner-accepted residual iOS risk, not an iPhone PASS. Stage 7.1 through Stage 7.4 are superseded for Stage 7 closure. Stage 7R.0 through Stage 7R.5 are complete locally; Stage 7R.5 hard gate and evidence reclosure passed `audit:stage-7`, `test:stage-7-lab-perf`, and `verify:stage-7:7.4`, and Stage 7.5 final closure is the current Stage 7 authority. Stage 7 is locally STAGE_7_CLOSED. Stage 5 remains closed; production remains `NO-GO`. Stage 7R authority: `docs/PHASE_85_STAGE_7R_SUPERSESSION_DECISION.json`; finding lock: `docs/PHASE_85_STAGE_7R_FINDING_LOCK.json`; iOS waiver authority: `docs/OWNER_IOS_VALIDATION_WAIVER_DECISION.md`.
...
Current continuity status, 2026-08-28: prior Stage 4 tracks remain closed locally, Stage 5 is closed locally, and Stage 6 is locally closed with explicit iPhone validation waiver. The owner permanently waived physical iPhone validation for this roadmap and future phases; iPhone remains `WAIVED_NOT_EXECUTED`, not PASS. Stage 7.1 through Stage 7.4 are historical implementation evidence only and are superseded for closure by Stage 7R.0. Stage 7R.1 rebuilt the deterministic harness, Stage 7R.2 recorded the trusted baseline, Stage 7R.3 remediated the shared/public/commercial `remediationPhase: "7.2"` set to zero open findings, and Stage 7R.4 remediated the dashboard/PWA `remediationPhase: "7.3"` set to zero open findings. R-405 is technically resolved locally. Push remains a separate operator action, and production remains `NO-GO` pending its independent gates.
