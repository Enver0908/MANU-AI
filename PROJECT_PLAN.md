...
Current Stage 4B-2 local RLS re-closure, 2026-07-13: Docker Desktop/local Supabase is available again for the current local RLS suite. `npx supabase db reset` passed and `npm run test:rls` passed 35/35 with 0 skipped after append-only RLS re-closure migration `20260713024000_phase_85_stage_4b2_rls_local_reclosure.sql` plus fixture/store corrections. Evidence: `docs/PHASE_85_STAGE_4B_2_RLS_LOCAL_RECLOSURE_EVIDENCE.md`. Production pilot remains `NO-GO`; R-405 remains open; real provider/channel/health-data paths remain closed; SQL buffer/EXPLAIN closure is not claimed.
