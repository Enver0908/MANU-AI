...
**Current AIya hosted deploy/live smoke status (2026-09-01):** Phase 3 preparation is recorded in `docs/AIYA_HOSTED_DEPLOY_LIVE_SMOKE_EVIDENCE.md` with action plan `docs/AIYA_HOSTED_DEPLOY_LIVE_SMOKE_ACTION_PLAN.md`. Deploy candidate HEAD is `dbad6dbb9dcadc42e140c5e4a448653420acbed9`. A new production build passed and the hosted artifact was generated as `hs-dbad6dbb9dca-2c32cf194421` with archive SHA-256 `723137e0a0f3702f1e43666ef7948ca1ee927201548e55fbb2972a6bcf46015c`. Initial deploy dry-run found a real release tooling bug: `release-manifest.json` was included in its own archive file-hash list and therefore failed extracted hash verification. This was fixed in `app/scripts/build-release-artifact.mjs`, with tests updated in `app/scripts/release-artifact.test.mjs` and `tools/hosted-sandbox/deploy/hosted-sandbox-deploy.test.mjs`; `npm run test:release-artifact` passed 1/1 and `node --test tools/hosted-sandbox/deploy/hosted-sandbox-deploy.test.mjs` passed 8/8. Hosted deploy dry-run now passes with `artifactMode:"archive"`. Real remote apply was not executed because this local session is missing the required secure deploy/SSH environment values: `MANU_HOSTED_DEPLOY_HOST`, `MANU_HOSTED_DEPLOY_USER`, `MANU_SSH_KNOWN_HOSTS_FILE`, `MANU_SSH_HOST_KEY_PIN`, `MANU_HOSTED_DEPLOY_APPROVED`, and `MANU_SMOKE_BASE_URL`. Live brand mismatch and live 500s remain open until a separately executed remote apply and live smoke complete. Production remains `NO-GO`; no remote migration, external setting change, production gate change, live provider/channel traffic, live billing, worker start, or real health-data processing was executed.
...
**Current AIya live parity and runtime smoke finding (2026-09-01):** Phase 1 read-only live smoke is recorded in `docs/AIYA_LIVE_PARITY_RUNTIME_SMOKE_EVIDENCE.md` with action plan `docs/AIYA_LIVE_PARITY_RUNTIME_SMOKE_ACTION_PLAN.md`. The local branch remains clean on `codex/production-readiness-stage-1` at `609f31089d10d6e51aee59fad013efa2fa3144e9`, but the live `https://aiyaworkspace.com/api/health/release` endpoint still reports deployed commit `d1e0b5f40e3a6e3b535e2a889ebf68025c5e548a` and release `hs-d1e0b5f40e3a-5ad2055fb26f`. Live public/admin surfaces and `/manifest.webmanifest` still expose `SiriusAI`, while the local active-surface brand scan is clean except for the allowed compatibility note in `app/src/lib/brand.ts`. Live `/app-install`, `/api/app-state`, and `/api/clients` return `500`; VPS logs are required to distinguish env, schema/RPC, and server bundle causes. Public DNS/TLS for `aiyaworkspace.com` and `admin.aiyaworkspace.com` are valid, and old `siriusai.store` HTTP/HTTPS routes return `410`. Supabase Auth sender display name remains unproven from public checks; the latest recorded external evidence still says `SiriusAI <no-reply@auth.aiyaworkspace.com>`. Production remains `NO-GO`; no deploy, remote migration, external setting change, production gate change, live provider/channel traffic, live billing, worker start, or real health-data processing was executed.
...
**Current Hosted Sandbox technical-debt authority (2026-08-28):** The old Hosted Sandbox Remediation v1.1 governance plan is superseded. Hosted Sandbox technical debt is `TECHNICAL_DEBT_CLOSED` by `docs/hosted-sandbox/evidence/HOSTED_SANDBOX_TECHNICAL_DEBT_CLOSURE_EVIDENCE.md`. Verification passed for hosted Node tests 23/23, targeted hosted Vitest 20/20, typecheck, lint with existing 77 warnings and 0 errors, production build, release artifact and hash verification, `npm run release:verify`, clean local Supabase reset, `npm run test:rls` 56/56 with 0 skipped, linked Supabase backup/encryption, isolated restore drill, remote migration apply through `20260826130000`, hosted cleanup apply with guard restored, manual remote deploy, exact public `/api/health/release` smoke, and rollback rehearsal. The scripted `apply-hosted-release.mjs` path is `PARTIAL` only because the remote helper was missing, and is superseded by the manual remote deploy evidence for this closure. Do not translate this technical debt closure into production readiness: production remains `NO-GO`; physical iPhone Safari/PWA remains `WAIVED_NOT_EXECUTED`, not PASS, under the permanent owner waiver.
