...
**Current Hosted Sandbox technical-debt authority (2026-08-27):** The old Hosted Sandbox Remediation v1.1 governance plan is superseded. Local code/evidence closure is recorded in `docs/hosted-sandbox/evidence/HOSTED_SANDBOX_TECHNICAL_DEBT_CLOSURE_EVIDENCE.md`. Local verification passed, including hosted Node tests 23/23, targeted hosted Vitest 20/20, typecheck, lint with existing 77 warnings, production build, release artifact, and `npm run release:verify`. Remote execution remains blocked/not-run because Docker daemon, Supabase CLI, `pg_dump`, `age`, hosted secrets, SSH known_hosts pin, and explicit remote approvals were unavailable. Do not claim hosted runtime closure or production readiness until those remote rows are run for real and recorded PASS. Production remains `NO-GO`; physical iPhone Safari/PWA remains `WAIVED_NOT_EXECUTED`, not PASS.
