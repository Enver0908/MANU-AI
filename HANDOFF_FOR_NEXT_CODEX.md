...
**Current public surface/auth/onboarding/PWA planning status (2026-09-04):** Phase 6 is locally complete. Evidence: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_PHASE_6_PWA_INSTALL_RESPONSIVE_A11Y_EVIDENCE.md`. Canonical plan: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_ACTION_PLAN.md`. AIya PWA remains installable at `/app-install`, network-only for auth/dashboard/API, and privacy-locked offline. Local Android Chrome and installed-PWA checks PASS on Playwright Chromium Pixel 5; TalkBack and iPhone Safari/PWA remain `WAIVED_NOT_EXECUTED` and are not PASS. The next eligible unit is Phase 7 only after explicit user approval. Production remains `NO-GO`.
