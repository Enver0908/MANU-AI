...
**Current public surface/auth/onboarding/PWA planning status (2026-09-04):** Phase 4 is locally complete. Evidence: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_PHASE_4_ADMIN_CUSTOMER_LIFECYCLE_EVIDENCE.md`. Canonical plan: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_ACTION_PLAN.md`. Allowlist admin session can list customers by email, invite a new customer as one command, close access with expected revision, and renew/reactivate the same tenant. Stripe subscription/purchase routes were not changed. The local reactivation SQL file was not applied remotely. The next eligible unit is Phase 5 only after explicit user approval. Production remains `NO-GO`.
