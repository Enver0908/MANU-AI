...
**Current public surface/auth/onboarding/PWA planning status (2026-09-04):** Phase 3 is locally complete. Evidence: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_PHASE_3_PASSWORD_ONBOARDING_EVIDENCE.md`. Canonical plan: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_ACTION_PLAN.md`. Daily customer login is email+password; magic link remains an explicit secondary/recovery option; invite onboarding shows the invited email read-only, sets a password on the authenticated user, then claims through the existing idempotent endpoint. `/app-install` replaced `/install` in the post-auth redirect allowlist. Open signup was not added. No Stripe, WhatsApp, Z.ai, deploy, production worker, live billing, or production gate changed. The next eligible unit is Phase 4 only after explicit user approval. Production remains `NO-GO`.
