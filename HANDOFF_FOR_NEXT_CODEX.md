...
**Current Hosted Sandbox technical-debt authority (2026-08-28):** The old Hosted Sandbox Remediation v1.1 governance plan is superseded. Local code/evidence closure is recorded in `docs/hosted-sandbox/evidence/HOSTED_SANDBOX_TECHNICAL_DEBT_CLOSURE_EVIDENCE.md`. Local verification passed, including hosted Node tests 23/23, targeted hosted Vitest 20/20, typecheck, lint with existing 77 warnings, production build, release artifact, `npm run release:verify`, clean local Supabase reset, and `npm run test:rls` 56/56 with 0 skipped. Remote hosted execution remains blocked/not-run because `pg_dump`, `pg_restore`, `age`, hosted secrets, SSH known_hosts pin, and explicit remote approvals were unavailable. Do not claim hosted runtime closure or production readiness until those remote rows are run for real and recorded PASS. Production remains `NO-GO`; physical iPhone Safari/PWA remains `WAIVED_NOT_EXECUTED`, not PASS.
...
- Remote migration/deploy/cleanup apply NOT executed; production NO-GO; iPhone WAIVED_NOT_EXECUTED.
