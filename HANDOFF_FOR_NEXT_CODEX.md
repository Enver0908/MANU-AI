...
**Current public surface/auth/onboarding/PWA planning status (2026-09-03):** Phase 2 is locally complete. Evidence: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_PHASE_2_TWO_HOUR_SESSION_EVIDENCE.md`. Canonical plan: `docs/PUBLIC_SURFACE_AUTH_ONBOARDING_PWA_ACTION_PLAN.md`. Authenticated web dashboard and PWA now use a two-hour server-authoritative idle window; hidden-tab polling does not extend the session; expiry redirects to `/login?next=/dashboard`. The local append-only migration was not applied remotely. No Stripe, WhatsApp, Z.ai, deploy, production worker, live billing, or production gate changed. The next eligible unit is Phase 3 only after explicit user approval. Production remains `NO-GO`.
