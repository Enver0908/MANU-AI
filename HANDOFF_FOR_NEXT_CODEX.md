...
**Current Hosted Sandbox Faz 1 authority (2026-08-25):** Faz 1 tenant isolation and demo fixture separation are implemented locally on branch `codex/stage-4c-remediation`. Hosted Supabase read/mutation paths no longer call `ensureDemoData()`; `resolveAccountTenantContext()` fail-closes ambiguous membership with `409 account_context_ambiguous`; demo login requires `NODE_ENV=development`, localhost, and `MANU_ALLOW_PUBLIC_DEMO_LOGIN=true`; cleanup script `app/scripts/hosted-sandbox-demo-cleanup.mjs` supports `--dry-run` default and explicit `--apply` with backup-manifest and remote-approval gates. Remote cleanup was not executed. Executor checks for targeted Vitest are pending in evidence. `test:rls` remains BLOCKED without Docker/local Supabase. Independent review is `NOT_REQUESTED`. Production remains `NO-GO`. Evidence: `docs/execution-governance/HOSTED_SANDBOX_PHASE_1_TENANT_ISOLATION_EVIDENCE.md`. Faz 0 verifier remains at commit `c62bcdd` binding until refreshed after Faz 1 commit. Next gated step: user commit approval for Faz 1, then explicit Faz 2 command.
