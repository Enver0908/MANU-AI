...
**Current AIya hosted deploy/live smoke status (2026-09-01):** Phase 3 is recorded in `docs/AIYA_HOSTED_DEPLOY_LIVE_SMOKE_EVIDENCE.md` with action plan `docs/AIYA_HOSTED_DEPLOY_LIVE_SMOKE_ACTION_PLAN.md`. The hosted VPS now serves HEAD `82ee3725076566304e9e0308632b2efe9d3b1deb` as release `hs-82ee37250765-2c32cf194421`. Live smoke passed for `/api/health/release`, `/`, `/login`, `/purchase`, `/app-install`, `/manifest.webmanifest`, admin `/admin`, active-surface AIya brand scan, and legacy `siriusai.store`/`www`/`admin` `410 Gone`; unauthenticated `/api/app-state` and `/api/clients` now return controlled `401`, not `500`. The official `apply-hosted-release.mjs` path is still blocked on the VPS because `/opt/manu-ai/tools/hosted-sandbox/deploy/deploy-hosted-release.mjs` is missing; the release was applied through a guarded manual fallback. A second live finding remains open: the Windows-built hosted artifact omitted Linux `sharp` optional runtime packages, so the deployed release required explicit remote install of `@img/sharp-linux-x64@0.35.3` and `@img/sharp-libvips-linux-x64@1.3.2` before route smoke passed. Supabase Auth sender display name remains owner-external evidence required. Production remains `NO-GO`; no remote migration, Supabase Auth edit, Resend edit, DNS edit, Stripe edit, WhatsApp edit, Z.ai edit, production gate change, live provider/channel traffic, live billing, worker start, or real health-data processing was executed.
