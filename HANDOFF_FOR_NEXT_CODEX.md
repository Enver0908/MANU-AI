...
**Current authority (2026-08-20):** Stage 6 accessibility remediation is verified and `S6-P4-A11Y` is closed. The authoritative desktop/Android-mobile axe run passes `12/12`; the final workspace matrix passes `23` tests with one desktop-only assertion skipped on mobile. Do not mark Stage 6 complete: Stage 6-specific physical iPhone Safari/PWA plus Android Chrome/PWA evidence is still absent, and older Stage 5 captures cannot substitute for it. Evidence: `docs/PHASE_85_STAGE_6_ACCESSIBILITY_REMEDIATION_EVIDENCE.md`. Stage 5 remains closed and production remains `NO-GO`.
